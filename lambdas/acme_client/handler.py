...
def generate_self_signed(csr_pem: bytes, domain: str) -> bytes:
    """
    Fallback: generate a self-signed certificate from the CSR when Pebble CA is unreachable.
    Valid for 90 days. Used for demo/dev when the Pebble EC2 instance isn't running.
    """
    ...
    cert = (
        ...
        .issuer_name(x509.Name([x509.NameAttribute(NameOID.COMMON_NAME, "Mississippi ITS Demo CA")]))
...
def handler(event, context):
    ...
    try:
        ...
        logger.info(f"Certificate obtained from Pebble CA for {domain}")
    except Exception as acme_err:
        logger.warning(f"Pebble CA unavailable ({acme_err}), generating self-signed certificate for demo")
        cert_pem = generate_self_signed(csr_pem, domain)
